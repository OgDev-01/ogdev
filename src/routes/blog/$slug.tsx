...
function BlogDetails() {
  ...
  const blogPostSchema = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `${SITE_URL}/blog/${blog.slug}_${blog.id}`,
    },
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: blog.title,
        item: `${SITE_URL}/blog/${blog.slug}_${blog.id}`,
      },
    ],
  };
    ...
    <>
      ...
      <div className="mx-auto max-w-3xl px-6 py-12 md:px-12 md:py-20">
        ...
        <article ref={articleRef}>
          ...
          {/* Table of Contents */}
          <FadeIn delay={225}>
            <TableOfContents content={blog.body_markdown} className="mb-10" />
          </FadeIn>
            ...
            <FadeIn delay={300}>
              <div className="mt-12 flex flex-wrap gap-2">
                {blog.tags.map((tag: string) => (
                  <span
                    key={tag}
                    className="rounded-full bg-primary-button/10 px-3 py-1 text-sm font-medium text-primary-button dark:bg-secondary-button/20 dark:text-primary-white/90"
                  >
                    #{tag}
                  </span>
                ))}
